...
template<class T>
MemoriaCompartida2<T>::MemoriaCompartida2() : shmId(0), ptrDatos(NULL) {
}

template<class T>
void MemoriaCompartida2<T>::crear(const std::string &archivo, const char letra) {
...
template<class T>
void MemoriaCompartida2<T>::liberar() {
...
template<class T>
MemoriaCompartida2<T>::MemoriaCompartida2(const std::string &archivo, const char letra) : shmId(0), ptrDatos(NULL) {
    key_t clave = ftok(archivo.c_str(), letra);
...
template<class T>
MemoriaCompartida2<T>::MemoriaCompartida2(const MemoriaCompartida2 &origen) : shmId(origen.shmId) {
...
template<class T>
MemoriaCompartida2<T>::~MemoriaCompartida2() {
...
template<class T>
MemoriaCompartida2<T> &MemoriaCompartida2<T>::operator=(const MemoriaCompartida2 &origen) {
...
template<class T>
void MemoriaCompartida2<T>::escribir(const T &dato) {
    *(this->ptrDatos) = dato;
}

template<class T>
T MemoriaCompartida2<T>::leer() const {
    return *(this->ptrDatos);
}

template<class T>
int MemoriaCompartida2<T>::cantidadProcesosAdosados() const {
    shmid_ds estado;
